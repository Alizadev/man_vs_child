...
public class RagdollHelper : MonoBehaviour {
	...
	void Start ()
	{
        ...
        Rigidbody[] allRigids = GetComponentsInChildren<Rigidbody>();
        foreach (Rigidbody rg in allRigids)
        {
		...
		foreach (Component c in components)
		{
			...
			if(bodyPart.transform.name != "Prop")
	...
	void LateUpdate()
	{
		...
		if (state==RagdollState.blendToAnim)
		{
			if (Time.time<=ragdollingEndTime+mecanimToGetUpTransitionTime)
			{
				...
				RaycastHit[] hits=Physics.RaycastAll(new Ray(newRootPosition,Vector3.down),layer);
				newRootPosition.y = 0;
				//newRootPosition = newRootPosition; //+ new Vector3(0,0.6f,0);
				...
				RaycastHit secHit;
				if(Physics.Raycast (ragdolledHipPosition ,Vector3.down ,out secHit ,layer)) {
					newRootPosition.y = secHit.point.y;
				}
				...
				var playerRot = Quaternion.FromToRotation(animatedDirection.normalized,ragdolledDirection.normalized);
